...
const { toFixedNumber, toFixedAdminCommission } = require("./commonHelper")
...
module.exports = {
    ...
    storeRegisterEmail: async (user) => {
        ...
        try {
            ...
            var sendmail = await awsSes.sendEmail(to, subject, body)
        } catch (err) {
    ...
    userOrderConfEmail: async (user, getStore, order, vendor) => {
        try {
            ...
            body = body.replace(/\[documentTitle\]/g, order.customOrderId + (user.name ? " - " + user.name : ""))
        ...
        try {
            ...
            generatePdf(body, async (err, result) => {
                ...
                if (err)
                    ...
                    var sendmail = await awsSes.sendEmail(to, subject, body, file)
                }
            })
    ...
    vendorForgotPasswordEmail: async (user) => {
        ...
        try {
            ...
            const getStore = await Store.findOne().populate({
                path: "logo",
            })
            const getTemplate = await Template.findOne({
    ...
    adminApproveVendorEmail: async (user, { password = "", username = "" }) => {
        if (!user) return
        try {
            let to = user.email
            ...
            let subject = getTemplate.subject
            subject = subject.replace(/\[storeName\]/g, "UZA Bulk")

            let body = getTemplate.body

            body = body.replace(/\[logo\]/g, env.apiBaseUrl + "/images/logo.png");
            body = body.replace(/\[storeName\]/g, "UZA Bulk");
            body = body.replace(/\[vendorName\]/g, user.name);
            body = body.replace(/\[email\]/g, user.email);
            body = body.replace(/\[username\]/g, username);
            body = body.replace(/\[password\]/g, password);
            body = body.replace(/\[loginLink\]/g, env.vendorPanelUrl);
            body = body.replace(/\[copyRightYear\]/g, new Date().getFullYear());
    ...
    htmlslipdata: async (user, order) => {
        ...
        const products = order.line_items && order.line_items.map(item => {
            ...
            //content = content.replace(/\[productImage\]/g, item.productImage)
            content = content.replace(/\[productName\]/g, item.productName)
            content = content.replace(/\[productQuantity\]/g, item.quantity)
            content = content.replace(/\[currency\]/g, getStore.currency.sign)
            content = content.replace(/\[productCost\]/g, toFixedAdminCommission(item.unitPrice, order?.adminCommission))
            content = content.replace(/\[productSubtotal\]/g, toFixedAdminCommission(item.amount, order?.adminCommission))
        ...
        body = body.replace(/\[email\]/g, user?.email || "")
        body = body.replace(/\[mobileNumber\]/g, (user?.countryCode || "") + " " + (user?.mobileNumber || ""))
        body = body.replace(/\[billingName\]/g, order.billingDetails ? order.billingDetails.name || "" : "")
        body = body.replace(/\[billingMobileNumber\]/g, order.billingDetails ? (order.billingDetails.countryCode + " " + order.billingDetails.mobileNumber) || "" : "")
        body = body.replace(/\[billingArea\]/g, order.billingDetails ? order.billingDetails.area || "" : "")
        body = body.replace(/\[billingHouseNo\]/g, order.billingDetails ? order.billingDetails.houseNo || "" : "")
        body = body.replace(/\[billingAddress\]/g, order.billingDetails ? order.billingDetails.address || "" : "")
        body = body.replace(/\[billingLandmark\]/g, order.billingDetails ? order.billingDetails.landmark || "" : "")
        body = body.replace(/\[billingAddressType\]/g, order.billingDetails ? order.billingDetails.addressType || "" : "")

        body = body.replace(/\[shippingName\]/g, order.shippingDetails ? order.shippingDetails.name || "" : "")
        body = body.replace(/\[shippingMobileNumber\]/g, order.shippingDetails ? (order.shippingDetails.countryCode + " " + order.shippingDetails.mobileNumber) || "" : "")
        body = body.replace(/\[shippingArea\]/g, order.shippingDetails ? order.shippingDetails.area || "" : "")
        body = body.replace(/\[shippingHouseNo\]/g, order.shippingDetails ? order.shippingDetails.houseNo || "" : "")
        body = body.replace(/\[shippingAddress\]/g, order.shippingDetails ? order.shippingDetails.address || "" : "")
        body = body.replace(/\[shippingLandmark\]/g, order.shippingDetails ? order.shippingDetails.landmark || "" : "")
        body = body.replace(/\[shippingAddressType\]/g, order.shippingDetails ? order.shippingDetails.addressType || "" : "")
        // body = body.replace(/\[storeAddress\]/g, order.billingDetails.address || "")

        body = body.replace(/\[products\]/g, products)
        body = body.replace(/\[currency\]/g, getStore.currency.sign)
        body = body.replace(/\[productTotal\]/g, toFixedAdminCommission(order.subTotal, order?.adminCommission))
        body = body.replace(/\[orderAmount\]/g, toFixedNumber(order.orderTotal))
        body = body.replace(/\[tax\]/g, toFixedNumber(order.taxAmount || 0))
        body = body.replace(/\[taxAmount\]/g, toFixedNumber(order.tax || 0))
        body = body.replace(/\[deliveryFee\]/g, toFixedNumber(order.deliveryFee || 0))
        return body;
    }
}
