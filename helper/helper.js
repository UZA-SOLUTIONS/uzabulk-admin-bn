...
module.exports = {
    ...
    getCardIcon: (type) => {
        ...
        let icon =
            "https://uzastagings3.s3.us-east-2.amazonaws.com/1704354128727visa.png";
        if (icons.length > 0) {
            icon = icons[0].link;
        }
        return icon;
    ...
    validatePriceTiers: (price_tiers) => {
        // Check if price tiers is an array
        if (!Array.isArray(price_tiers)) {
            return { valid: false, message: 'Price tiers should be an array.' };
        }

        for (let i = 0; i < price_tiers.length; i++) {
            const tier = price_tiers[i];

            // Check if startQuantity and price are numbers
            if (typeof tier.startQuantity !== 'number' || isNaN(tier.price)) {
                return { valid: false, message: `Tier at index ${i} has invalid type for startQuantity or price.` };
            }

            // Check if startQuantity is a positive integer
            if (!Number.isInteger(tier.startQuantity) || tier.startQuantity <= 0) {
                return { valid: false, message: `Tier at index ${i} has invalid startQuantity.` };
            }

            // Check if price is a positive number
            if (tier.price <= 0) {
                return { valid: false, message: `Tier at index ${i} has non-positive price.` };
            }

            // Check for overlapping tiers
            if (i > 0) {
                const prevTier = price_tiers[i - 1];
                if (tier.startQuantity <= prevTier.startQuantity) {
                    return { valid: false, message: `Tier at index ${i} overlaps with previous tier.` };
                }
            }
        }

        // Sort price tiers by startQuantity
        price_tiers.sort((a, b) => a.startQuantity - b.startQuantity);

        return { valid: true, message: 'All price tiers are valid.' };
    }
