...
module.exports = {
    ...
    viewCategoryListByFilter: async (req, res) => {
        try {
            ...
            const countdata = await Category.countCategories(obj);

            Category.uzaCategories(obj, sortByField, sortOrder, paged, pageSize, (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                }
                return res.json(helper.showSuccessResponseCount('CATEGORY_DATA', resdata, countdata));
            });
        }
        catch (err) {
            console.log("err", err);
    ...
    categoryListByLevel: async (req, res) => {
        try {
            ...
            let categories = await Category.find(query, "parent catName catImage").sort({ sortOrder: 1 });

            res.json(helper.showSuccessResponse('CATEGORY_DATA', categories));
