...
const storeType = require("../models/storeTypeTable");
const { updateProductIndex } = require('../lib/elasticSearch.js');
...
module.exports = {
    ...
    createProductData: async (req, res) => {
        try {
            ...
            if (req.isVendor) {
                data.vendor = req.userId;
                const vendorInfo = await userTable.getVendorInfo(req.userId);
                data.region = vendorInfo?.region || "CH";
            }
            else if (data?.vendor) {
                const vendorInfo = await userTable.getVendorInfo(data?.vendor);
                data.region = vendorInfo?.region || "CH";
            }
            ...
            Product.addProduct(data, (err, resdata) => {
                if (err) {
                    ...
                    updateProductIndex(resdata[0]._id);
                    return res.json(helper.showSuccessResponse('DATA_ADDED_SUCCESS', resdata[0]));
                }
            });
        } catch (error) {
            console.log("errorsss", error);
    ...
    viewProductByGroceryVendor: async (req, res) => {
        try {
            ...
            Product.getProductByIdGrocery(product_id, async (err, resdata) => {
                if (err) {
                    console.log("err", err);
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
                    if (resdata === null) {
                        ...
                        const storeData = await StoreType.getUzaStore({ currency: 1 });
                        resdata._doc.exchangeRate = storeData?.currency;
                        res.json(helper.showSuccessResponse('DATA_SUCCESS', resdata));
                    }
                }
            });
        ...
        try {
            ...
            Product.updateProduct(data, (err, resdata) => {
                if (err) {
                    ...
                    updateProductIndex(resdata._id);
    ...
    viewProductListByFilter: async (req, res) => {
        try {
            const { orderBy, order, page, limit, storeTypeId, storeTypeName, vendor, fields, search, categories, oldCount } = req.body
            ...
            //     }
            // });
            Product.getUzaProductsList(obj, pageSize, sortByField, sortOrder, paged, async (err, resdata) => {
                if (err) {
                    return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                } else {
                    ...
                    if (countdata === pageSize || ((paged - 1) * pageSize === 0)) {
                        countdata = await Product.countDocuments(obj);
                    }
