...
const { enrichOrderLineItems } = require('../helper/enrichOrderLineItems');
...
const { alibabaCreateOrder, getFreightTemplateList } = require('../helper/alibabaOrderService.js');
const {
...
module.exports = {
    ...
    userLogin: async (req, res) => {
        try {
            ...
            // await User.updateOne({ email: data.email, }, { $set: { password: (await Utils.hashPassword("123456")).hashedPassword } });
            const getUser = await User.findOne({ email: data.email, role: data?.role || { $in: ["ADMIN", "VENDOR", "STAFF"] }, status: { $nin: ["archived", "temp"] } })
    ...
    userForgotPassword: async (req, res) => {
        try {
            ...
            let role = data?.role ? data?.role : { $in: ["ADMIN", "VENDOR", "STAFF"] };

            data.email = data.email.trim().toLowerCase();

            let getUser = await User.findOne({ status: { $ne: "archived" }, email: data.email, role: role });
    ...
    updateUserProfile: async (req, res) => {
        try {
            ...
            if (user.email) {
                ...
                if (data.email.toString().toLowerCase() != user.email.toString().toLowerCase()) {
                    if (is_demo) {
            ...
            if (user.mobileNumber) {
                if (data.mobileNumber.toString() != user.mobileNumber.toString()) {
                    ...
                    const getUser = await User.findOne({ mobileNumber: data.mobileNumber, countryCode: data.countryCode, role: { $in: ["ADMIN", "VENDOR", "STAFF"] }, status: { $nin: ["archived", "temp"] } });

                    if (getUser != null) {
                        return res.json(helper.showValidationErrorResponse('MOBILE_ALREADY_EXISTS'));
                    }
                }
    ...
    addUserByAdmin: async (req, res) => {
        try {
            ...
            data.storeType = req.user?.storeType;
            // data.store = store.storeId;
            ...
            if (req.user.role === "ADMIN") {
                ...
                if (!["USER", "WHOLESALER", "STAFF"].includes(data.role)) {
        ...
        try {
            ...
            if (data.email && data.email.trim().toString() != getUser.email) {
                ...
                if (data.role === "USER") {
                    ...
                    let emailCheck = await User.findOne({ email: data.email, role: "USER", status: { $ne: "archived" } });
    ...
    getUserDetailsById: async (req, res) => {
        try {
            ...
            let user_obj = { _id: ObjectId(id), role: { $in: ["USER", "WHOLESALER", "STAFF"] } }
    ...
    getRestaurantDetailsById: async (req, res) => {
        try {
            ...
            let user_obj = { _id: ObjectId(id), role: "VENDOR" }
                ...
                if (err) {
                    ...
                    if (resdata) {
                        ...
                        let [latestOrders, totalOrders, orderAvgValue, vendorEarning, productCount, categoryCount, addonCount, attributeCount, productReview] = await Promise.all([
                            Order.aggregate([{ $match: { vendor: ObjectId(resdata._id), orderStatus: { $ne: "archived" } } }, { $lookup: { from: 'users', localField: 'user', foreignField: '_id', as: 'customerDetails' } }, { $unwind: { path: "$customerDetails", preserveNullAndEmptyArrays: true } }, { $project: { customOrderId: 1, billingDetails: 1, shippingDetails: 1, line_items: 1, vendor: 1, user: 1, isDriverAssign: 1, isDriverArrivedAtPickup: 1, isOrderMarkReady: 1, subTotal: 1, tax: 1, taxAmount: 1, tip: 1, tipAmount: 1, discountTotal: 1, deliveryFee: 1, orderTotal: 1, vendorEarning: 1, orderStatus: 1, date_created_utc: 1, vendorDetails: { name: 1, address: 1, profileImage: 1 }, customerDetails: { name: 1, email: 1, countryCode: 1, mobileNumber: 1, profileImage: 1, role: 1 } } }, { $sort: { date_created_utc: -1 } }, { $limit: 10 }]),
                            ...
                            Category.aggregate([{ $match: { status: 'active', parent: 'none' } }, { $group: { _id: null, count: { $sum: 1 } } }]),
                            Addon.aggregate([{ $match: { vendor: ObjectId(resdata._id), status: 'active' } }, { $group: { _id: null, count: { $sum: 1 } } }]),
                            Attribute.aggregate([{ $match: { vendor: ObjectId(resdata._id), status: { $ne: "archived" } } }, { $group: { _id: null, count: { $sum: 1 } } }]),
                ...
                if (err) {
                    ...
                    if (data._id.length === 1) {
                        ...
                        if (data.status === "approved") {
                            ...
                            getUser = await User.findOneAndUpdate(
                                { _id: ObjectId(data._id[0]) },
                                { password: (await Utils.hashPassword(password)).hashedPassword },
                                { new: true }
                            );
                        }
                        ...
                        if (!getUser) {
                            ...
                            if (data.status === "approved") {
                                emailService.adminApproveVendorEmail(getUser, { password, username: getUser.email });
    ...
    addRestaurantByAdmin: async (req, res) => {
        try {
            ...
            data.avgRating = 0
            data.reviewCount = 0
            let getUser = await User.findOne({ email: data.email, role: { $in: ["ADMIN", "STAFF", "VENDOR"] }, status: { $nin: ["archived", "temp"] } });
            ...
            data.state = data?.businessState;
            data.country = data?.businessCountry;
            data.region = data?.businessRegion;
            data.postalCode = data?.businessPostalCode;
            data.storeType = user?.storeType;
    ...
    getOrdersByStoreAdmin: async (req, res) => {
        try {
            ...
            if (req.isVendor) {
                obj.vendor = ObjectId(req.userId);
            }
            ...
            if (data.search) {
                const { buildOrderSearchOrConditions } = require('../helper/orderListQuery');
                const searchOr = await buildOrderSearchOrConditions(data.search);
                if (searchOr?.length) {
                    obj.$or = searchOr;
                }
            }
    ...
    changeOrderStatus: async (req, res) => {
        try {
            ...
            if (!data?._id) return res.json(helper.showDatabaseErrorResponse("ORDER_ID_REQUIRED"));
            if (!data?.orderStatus) return res.json(helper.showDatabaseErrorResponse("ORDER_STATUS_REQUIRED"));
            if (!["pending",
                "confirmed",
                "ready_for_shipment",
                "shipped",
                ...
                "rejected",
                "cancelled",
                "archived",].includes(data?.orderStatus)) return res.json(helper.showDatabaseErrorResponse("INVALID_ORDER_STATUS"));

            if (req.user?.role === "ADMIN" && data?.orderStatus === "confirmed") {
                const orderDetails = await Order.uzaOrderDetail(data._id);
                if (orderDetails && !orderDetails?.alibabaOrderId) {
                    const alibabOrders = [];
                    for (const order of orderDetails?.line_items) {
                        if (order?.offerId) {
                            alibabOrders.push({
                                specId: order?.specId || "",
                    ...
                    if (alibabOrders?.length) {
                        const orderResponse = await alibabaCreateOrder(alibabOrders);

                        if (orderResponse?.success) {
                            await Order.updateOne({ _id: ObjectId(data._id) }, {
                                $set: {
                                    alibabaOrderId: orderResponse?.result?.orderId,
                                    alibabaOrderResponse: orderResponse?.result,
                                }
                            });
                        }
                        else {
                            await Order.updateOne({ _id: ObjectId(data._id) }, {
                                $set: {
                                    alibabaErrorMessage: orderResponse?.message,
                                }
                            });

                            return res.json(helper.showDatabaseErrorResponse("ALIBABA_RESPONSE_ERROR", orderResponse?.message));
                        }
                    }
                }
            ...
            const existingOrder = await Order.getOrderByIdAsyncForRes(data._id);
            if (!existingOrder) {
                return res.json(helper.showDatabaseErrorResponse("INVALID_ORDER_ID"));
            }

            const trackingHistory = mergeTrackingHistory(
                existingOrder.trackingHistory,
                data.orderStatus
            );

            Order.updateOrderVendorNew({
                ...
                if (err) {
                    ...
                    await syncTrackingStatusToLogistics({
                        orderId: data._id,
                        orderStatus: data.orderStatus,
                        orderDoc: resdata || existingOrder || {},
    ...
    /** 1688 freight templates (Alibaba Open Platform logistics API). */
    getAlibabaFreightTemplates: async (req, res) => {
        try {
            const { templateId, querySubTemplate, queryRate } = req.body || {};
            const response = await getFreightTemplateList({
                templateId,
                querySubTemplate,
                queryRate,
            });
            ...
            if (!response.success) {
                return res.json(
                    helper.showValidationErrorResponse(
                        'ALIBABA_FREIGHT_TEMPLATE_ERROR',
                        response.message || response.errorMsg
                    )
                );
            }

            return res.json(helper.showSuccessResponse('DATA_SUCCESS', response.result));
        } catch (err) {
            console.log('getAlibabaFreightTemplates err', err);
            res.json(helper.showInternalServerErrorResponse('INTERNAL_SERVER_ERROR'));
        }
    },

    /** Save 1688 logistics trace request params (trade_id, carrier, waybill). */
    updateOrderAlibabaLogistics: async (req, res) => {
        try {
            ...
            const alibabaLogistics = {
                ...(order.alibabaLogistics || {}),
                tradeId: tradeId
                    ? String(tradeId)
                    : order.alibabaLogistics?.tradeId || order.alibabaOrderId || '',
                logisticsCompanyCode:
                    logisticsCompanyCode != null
                        ? String(logisticsCompanyCode)
                        : order.alibabaLogistics?.logisticsCompanyCode || '',
                waybillNumber:
                    waybillNumber != null
            ...
            await Order.updateOne({ _id: orderId }, { $set: { alibabaLogistics } });

            return res.json(
                helper.showSuccessResponse('DATA_SUCCESS', {
                    alibabaLogistics,
                    traceQuery: buildTraceQuery({ ...order, alibabaLogistics }),
                })
            );
        } catch (err) {
    ...
    /** Poll alibaba.logistics.trace.get for one order (admin manual sync). */
    syncOrderAlibabaLogisticsTrace: async (req, res) => {
        try {
            const orderId = req.params._id;
            if (!orderId) {
                return res.json(helper.showValidationErrorResponse('ORDER_ID_REQUIRED'));
            ...
            const result = await pollOrderLogisticsTrace(orderId);
            if (!result.success) {
                return res.json(
                    helper.showValidationErrorResponse(
                        'ALIBABA_LOGISTICS_TRACE_ERROR',
                        result.message || result.errorMsg
                    )
                );
            }

            const order = await Order.findById(orderId)
                .select('alibabaLogistics trackingHistory orderStatus')
                .lean();

            return res.json(
                helper.showSuccessResponse('DATA_SUCCESS', {
                    alibabaLogistics: order?.alibabaLogistics,
                    trackingHistory: order?.trackingHistory,
                    orderStatus: order?.orderStatus,
                })
            );
        } catch (err) {
            console.log('syncOrderAlibabaLogisticsTrace err', err);
            res.json(helper.showInternalServerErrorResponse('INTERNAL_SERVER_ERROR'));
        }
    },
            ...
            Order.getOrderforpdf(obj, async (err, resdata) => {
                if (err) {
                    ...
                    try {
                        ...
                        await page.setContent(get_htmldata);
                        ...
                        // Convert Buffer to Stream and Pipe Response
        ...
        try {
            ...
            Order.getOrderByStoreId(order_obj, async (err, resdata) => {
                if (err) {
                    ...
                    const enrichedLineItems = await enrichOrderLineItems(resdata.line_items);
                    resdata.set("line_items", enrichedLineItems, { strict: false });
    ...
    getStoreByDomain: async (req, res) => {
        try {
            ...
            if (process.env.NODE_ENV == "staging") {
                data.domain = 'main.uza-staging.com';
            } else {
    ...
    getOrdersForCustomerAndDriver: async (req, res) => {
        try {
            ...
            const { applyOrderDateRange } = require('../helper/orderListQuery');
            applyOrderDateRange(obj, data.startDate, data.endDate);
...
// (async () => {
//     console.log(JSON.stringify(await getProductDetail('769145486520'), null, 2))
// })();