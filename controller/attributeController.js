...
module.exports = {
    ...
    addAttributeData: async (req, res) => {
        try {
            ...
            if (termsData && termsData.length > 0) {
                ...
                Attribute.addAttribute(data, async (err, resdata) => {
                    if (err) {
                        return res.json(helper.showDatabaseErrorResponse("INTERNAL_DB_ERROR", err));
                    } else {
                        ...
                        for (const term of termsData) {
                            let getTerm = await AttributeTerm.create({ name: term.name, image: term?.image || "", attribute: resdata._id, status: 'active' });
                            if (getTerm != null) {
    ...
    updateAttributetData: async (req, res) => {
        try {
            ...
            await AttributeTerm.deleteMany({ attribute: data._id });
            if (data.terms && data.terms.length > 0) {
                data.terms = data.terms;
                let terms = [];
                for (let term of data.terms) {
                    let getTerm = await AttributeTerm.create({ name: term.name, image: term?.image || "", attribute: data._id, status: 'active' });
                    if (getTerm != null) {
                        if (!terms.includes(getTerm._id.toString())) {
                            terms.push(getTerm._id.toString());
                        }
