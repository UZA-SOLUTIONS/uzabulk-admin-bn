        ...
        try {
            ...
            Role.getRoleById(attribute_id, (err, resdata) => {
                if (err) {
                    ...
                    const oldKeys = resdata.permissions.map((role) => role.type);
                    const currentRoles = Config.ACCESS_LIST?.map((role) => role.type);
                    resdata.permissions = resdata.permissions.filter((role) => currentRoles.includes(role.type));
                    currentRoles.forEach((role, index) => {
                        if (!oldKeys.includes(role)) {
                            resdata.permissions.push(Config.ACCESS_LIST[index]);
                        }
                    });
