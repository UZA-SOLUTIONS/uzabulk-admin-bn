...
const ORDER_LIST_MAX_FETCH = 5000;

const orderListPaginationStages = (paged, pageSize) => {
    const size = parseInt(pageSize, 10);
    const page = Math.max(1, parseInt(paged, 10) || 1);
    if (!size || size <= 0) {
        return [{ $limit: ORDER_LIST_MAX_FETCH }];
    }
    const limit = Math.min(size, ORDER_LIST_MAX_FETCH);
    return [
        { $skip: (page - 1) * limit },
        { $limit: limit },
    ];
...
let orderSchema = mongoose.Schema({
    ...
    orderStatus: {
        type: String, enum: ["pending", "confirmed", "ready_for_shipment", "shipped", "delayed", "out_for_delivery", "inroute", "completed", "refunded", "rejected", "cancelled", "archived"],
        default: "pending"
    },
...
module.exports.getOrderByIdAsyncForRes = async (id, callback) => {
    return orderTable.findById(id)
...
module.exports.uzaOrderDetail = async (id) => {
    return orderTable.findById(id);
}
...
module.exports.getNewOrdersWithFilter = function (obj, sortByField, sortOrder, paged, pageSize, callback) {
    ...
    orderTable.aggregate([
        ...
        ...orderListPaginationStages(paged, pageSize),
        ...
        { $unwind: { path: "$storeType", preserveNullAndEmptyArrays: true } },
        {
            $project: {
                ...
                customerDetails: { _id: 1, name: 1, address: 1, email: 1, mobileNumber: 1, role: 1 },
            }
        }
    ], callback);
}
    ...
    return orderTable.aggregate([
        ...
        { $unwind: { path: "$customerDetails", preserveNullAndEmptyArrays: true } },
        {
            $project: {
                ...
                line_items: 1, checkInDate_utc: 1, coupon: 1, checkOutDate_utc: 1,
                ...
                taxAmount: 1, tip: 1, tipAmount: 1, discountTotal: 1, deliveryFee: 1, orderTotal: 1, vendorEarning: 1, adminCommission: 1,
                deliveryBoyEarning: 1, orderStatus: 1, date_created_utc: 1, vendorDetails: { name: 1, address: 1, userLocation: 1, profileImage: 1 },
                customerDetails: {
                    _id: 1, name: 1, email: 1, countryCode: 1, mobileNumber: 1, profileImage: 1, role: 1,
    ...
    orderTable.aggregate([
        ...
        { $unwind: { path: "$customerDetails", preserveNullAndEmptyArrays: true } },
        {
            $project: {
                ...
                customerDetails: { name: 1, email: 1, countryCode: 1, mobileNumber: 1, profileImage: 1, avgRating: 1, role: 1 },
