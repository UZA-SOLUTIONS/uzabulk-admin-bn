...
productSchema.index({ name: "text", vendor: 1, status: 1, topCategoryId: 1, secondCategoryId: 1, thirdCategoryId: 1 }, { name: "name_text_search" });
...
productSchema.index({ vendor: 1, status: 1 }, { name: "vendor_product_listing" });
...
module.exports.getProductByCategoryIdAsync = (condition, categoryId, filter, callback) => {
  return productTable
    .find(
      ...
      "name price bestSeller compare_price featured_image addons pricingType short_description"
    )
...
module.exports.getProductByConditionAsync = (condition, pageOptions) => {
  return productTable.find(
    condition,
    "name price bestSeller compare_price featured_image categories vendor addons average_rating rating_count short_description variations storeType"
  )
...
module.exports.getProductById = (id, callback) => {
  productTable
    ...
    .populate({ path: "topCategoryId", match: { status: "active" } })
    .populate({ path: "secondCategoryId", match: { status: "active" } })
    .populate({ path: "thirdCategoryId", match: { status: "active" } })
...
module.exports.getProductByIdGrocery = (id, callback) => {
  productTable
    .findById(id)
    .populate({ path: "categories" })
    .populate({ path: "topCategoryId", select: { catName: 1, level: 1, parent: 1 }, match: { status: "active" } })
    .populate({ path: "secondCategoryId", select: { catName: 1, level: 1, parent: 1 }, match: { status: "active" } })
    .populate({ path: "thirdCategoryId", select: { catName: 1, level: 1, parent: 1 }, match: { status: "active" } })
...
module.exports.getUzaProductsList = (
  obj,
  pageSize,
  sortByField,
  sortOrder,
  ...
  productTable.find(obj, { name: 1, _id: 1, bestSeller: 1, status: 1, createdAt: 1, updateAt: 1, vendor: 1, offerId: 1 }).skip((paged - 1) * pageSize).limit(parseInt(pageSize)).exec(callback);
}
...
async function projection() {
  return { "icons": constants.AIRBNB_ICONS, name: 1, status: 1, price: 1, address: 1, average_rating: 1, pricingType: 1 };
