...
module.exports = {
    ...
    createProductDefault: async (req, res, next) => {
        ...
        if (data.price_tiers && data.price_tiers.length) {
            const { valid, message } = helper.validatePriceTiers(data.price_tiers);
            if (valid == false) {
                return res.json(helper.showValidationErrorResponse(message));
            }
        };
    ...
    createProductByGroceryVendor: async (req, res, next) => {
        ...
        // if (!data.featureAttribute || !data.featureAttribute?.length) {
        //     return res.json(helper.showValidationErrorResponse('FEATURE_ATTRIBUTE_IS_REQUIRED'));
        // };
