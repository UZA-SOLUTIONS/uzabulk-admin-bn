...
module.exports = {
    ...
    storeSignup: async (req, res, next) => {
        ...
        if (!data.plan) {
            return res.json(helper.showValidationErrorResponse('PLANS_DETAILS_IS_REQUIRED'));
        }

        if (!data.plan.planType) {
            return res.json(helper.showValidationErrorResponse('PLANS_TYPE_IS_REQUIRED'));
        }

        if (!data.plan.interval) {
            return res.json(helper.showValidationErrorResponse('PLANS_INTERVAL_IS_REQUIRED'));
        }

        if (!data.plan.productType) {
            return res.json(helper.showValidationErrorResponse('PLANS_PRODUCT_TYPE_IS_REQUIRED'));
        }
    ...
    storeImageUpload: async (req, res, next) => {
        ...
        if (!data.type || data.type != "storeGallery") {
    ...
    updateUserProfile: async (req, res, next) => {
        ...
        if (!data.countryCode) {
            return res.json(helper.showValidationErrorResponse('COUNTRY_CODE_IS_REQUIRED'));
        }

        next();
    },

    enableVendorNotifications: async (req, res, next) => {
        let data = req.body;
